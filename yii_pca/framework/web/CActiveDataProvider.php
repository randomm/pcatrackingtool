...
class CActiveDataProvider extends CDataProvider
{
	...
	//	print_r($baseCriteria);
		if(($sort=$this->getSort())!==false)
		{
			// print_r($this->getSort());
	...
	/**
	 * Fetches the data with no limit from the persistent data storage.
	 * @return array list of data items
	 */
	...
	protected function fetchAllKeys()
	{
		$keys=array();
		foreach($this->getAllData() as $i=>$data)
		{
			$key=$this->keyAttribute===null ? $data->getPrimaryKey() : $data->{$this->keyAttribute};
			$keys[$i]=is_array($key) ? implode(',',$key) : $key;
		}
