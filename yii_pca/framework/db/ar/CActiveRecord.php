...
abstract class CActiveRecord extends CModel
{
	...
	public function returnSectorScopes($queryString)
	{
		$extendBracket = "";
		$sector_ids = "";
		if ($this->tableName() == "tbl_pca") $extendBracket = ")";
		else if ($this->tableName() == "tbl_target") $extendBracket = ")";
		if (Yii::app()->user->getId())
		{			
			$user_id = Yii::app()->user->getId();
		
			$sector_users = SectorUser::model()->findAll("user_id = $user_id");
			
			foreach ($sector_users as $key => $value) {				
				$sector_ids .=  $value['sector_id'].",";
			}

			 $sector_ids = substr($sector_ids, 0, -1);

		}else $sector_ids = "999";	
		$condition = $queryString." ($sector_ids)".$extendBracket;
	...
	public function getRelatedActivities($relation, $display, $links=NULL)
	{
		
		//size of relation object
		 $size = sizeof($relation);
		 $activity = "";
		// line break to display more than multiple attributes
		 if(substr($display, -1) == ";") // excel
		 {
		 	$delimeter = "\n";
		 	$display = substr($display, 0, -1);
		 }
		 else if (strpos($_SERVER['REQUEST_URI'],"admin") !== false) // grid view
		 {
		 	$delimeter = "\n";
		 }else $delimeter =  "<br/>"; // detail view
		 ...
		 for ($i=0;$i<$size;$i++)
		 {
		 	 ...
		 	 { 
		 	 	...
		 	 	foreach ($links as $key => $value) $relation1 = $relation1->$value;
		 		$activity .= $relation1[$display].$delimeter;

			 }else 
			 {
			 	$display1 = explode(',', $display);
			 	foreach ($display1 as $dp) {
			 		$activity .= " ".$relation[$i][$dp]." -";	# code...
			 	}
	...
	{
	//	print_r($this->tableSchema->rawName.'-'.'find');
	...
	{
	//	print_r($this->tableSchema->rawName.'-'.'findAll');
	...
	{
	//	print_r($this->tableSchema->rawName.'-'.'findByPk');
