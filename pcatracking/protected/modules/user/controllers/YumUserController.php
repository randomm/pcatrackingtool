	...
	public function actionCreate() {
		...
		if(isset($_POST['YumUser'])) {
			...
			if(!$user->hasErrors()) {
				...
				if (isset($_POST['YumUser']['sectors'])) 
				{
					$user->sectors = $_POST['YumUser']['sectors'];
				...
				if(!$user->hasErrors() && !$passwordform->hasErrors()) {
					$user->saveWithRelated($relatedSectors);
					if(isset($profile)) {
						$profile->user_id = $user->id;
	...
	public function actionUpdate() {
		...
		if(isset($_POST['YumUser'])) {
			...
			if(!$user->hasErrors()) {
				...
				if(!$passwordform->hasErrors() && $user->saveWithRelated($relatedSectors)) {
					if(isset($profile)) 
						$profile->save();
