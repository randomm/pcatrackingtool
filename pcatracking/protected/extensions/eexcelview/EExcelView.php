...
Yii::import('zii.widgets.grid.CGridView');
	...
	*/	
	class EExcelView extends CGridView
	{
		...
		//Document properties
		public $creator = 'PCA Tracking Tool';
		public $title = null;
		...
		public function renderRow($row)
		{
			...
			foreach($this->columns as $n=>$column):
				if($column->value!==null) {
					//This is a variable to save multiple values of model relations and 
					//we set it to 0 for each relation attribute to start over with saving values
					//$modelRelations = 0;

					//if the value of the column has the part "data->" in it then evaluate it
					//print_r($column->value);
					//echo "<br>";
					$value=$this->evaluateExpression($column->value ,array('row'=>$row,'data'=>self::$data[$row]));
					//print_r($value);
					// if (strpos($column->value, "data->") !== false){

					// 	$value=$this->evaluateExpression($column->value ,array('row'=>$row,'data'=>self::$data[$row]));

					// } else {
					// 	//don't evaluate it, it has already been evaluated through a columns_to_display function in CController
					// 	//this is for Has Many relations that cannot be evaluated by cgrdiview
					// 	$value=$column->value;
					// } 
					//This part is needed to get all values from the model relations array and return them as string 
					//to be attached later as string to the overall $value variable, which contains the model attributes and values
  			...
  			// 			$modelRelations .= $v.", ";
				...
				//Because $value is an array and the explode method below expects a string we need to convert it to string
				//$value = (string)$value;

				//Because part of the $value contains and array, which is created from the relations of the model, then we need to set it to 0 
				//so that further below we can attach the model Relatins values as string
				// if ( substr_count($value, "Array") > 0)
				// $value = "";
				// $value = $value.$modelRelations;
				//print_r($value);
				$dateF= explode("-", $value);
				//print_r($dateF);
				$c1=count($dateF);
