$(document).ready(function() {
	...
	$('#PcaReport_pca_id').click(function(){
		var pca_id = $(this).val();
		var pca_title = $(this).find("option:selected").html();
		...
		jQuery.ajax({
			...
			'success':function(data){//console.log(data[1]);
				...
				var targets ="";
				console.log(pca_title);
				if (data)
				{

					for(var i=0; i<data.length; i++){
						//console.log(data[i]['total']);
						targets += "<h4>Pca Indicator "+(i+1)+"</h4>";
						targets += "<p><strong>Indicator: </strong>"+data[i]['target_name']+"</p>";
				...
				else $('#targetBen #pcaTargets').html(pca_title+" does not have any Indicators");					
			}});
