$(document).ready(function() {
	...
	jQuery('body').on('change','.region',function(){
		...
		jQuery.ajax({
			...
			'beforeSend': beforeSendAjax(), 
			'url':'/pcatracking/pca/returnparent',
			'data':{'fk_val':$('#region_id').val(),'current_model':'locality','pk':'locality_id','name':'name','fk_name':'region_id'},
			'cache':false,
			'success':function(html){afterSendAjax();jQuery("#locality_id").html(html);}});
		return false;
	});
